...
const MatchAlgo = ({matrixCount}) => {

  const [mistakes, setMistakes] = useState(0)
  const [press, setPress] = useState(0)
  const [matrix, setMatrix] = useState(matrixCount)
  ...
  useEffect(() => {
    ...
    let random=(matrix*matrix)/2
    getColored(String(parseInt(random)+1),"new")
  }, []);
  ...
  function Move(txt) {
    ...
    if (JSON.stringify(winningArray) === JSON.stringify(array)) {
      if (press < 10 && matrix === 5) {
        alert("Won\nWell DOne you won 10 rupee paytem Cash")
  ...
  return (
    <ScrollView>
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center", marginVertical: 10 }} >
        ...
        <View style={{ flexDirection: "row", width: "100%", margin: 20, alignItems: "center", justifyContent: 'center' }} >
            ...
            <View style={{ flexDirection: "row" }} >

              {
                winningArray.slice(matrix * index, matrix * (index + 1)).map((item, index) => {
                  return (
                    <>
                      <View  style={{ borderColor: "purple", backgroundColor: 'transparent', height: 200 / matrix, width: 200 / matrix, borderRadius: 10, borderWidth: 1, justifyContent: 'center', alignItems: "center" }} >
